...
lookupZipCode query =
  let toUrl =
        if String.length query == 5 && String.all Char.isDigit query
          then succeed ("http://mockapi.dev:9090/wallets/" ++ query)
          else fail "Give me a valid US zip code!"
  in
      toUrl `andThen` (mapError (always "Not found :(") << Http.get wallets)


wallets : Json.Decoder (List String)
wallets =
  let wallet =
        Json.object3 (\id name bulkAccountId-> id ++ ", " ++ name ++ " - " ++ bulkAccountId)
          ("id" := Json.string)
          ("name" := Json.string)
          ("bulk_account_id" := Json.string)
  in
      "wallets" := Json.list wallet
